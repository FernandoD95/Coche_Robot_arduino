...
//Modificacion haciendo uso de fototransistores para contar los pasos dados por el motor
#if ARDUINO >= 100
  #include "Arduino.h"
#else
  #include "WProgram.h"
...
ControlMotor::ControlMotor(int MD1,int MD2,int MI1,int MI2,int PWMD,int PWMI, int F1, int F2, float KpI)
{
   ...
   pinMode(F1, INPUT);   // Valores del fotosensor 1
   pinMode(F2, INPUT);   // Valores del fotosensor 2
...
void ControlMotor::Actualizar_Paso(){
  Sensor1[0]=analogRead(F1);
  Sensor2[0]=analogRead(F2);
  if(abs(Sensor1[0]-Sensor1[1])<20){
            if(Sensor1[0]-Sensor1[1]<0)
                SB1[0]=1;
            else 
                SB1[0]=0;
        }
        if(SB1[1]!=SB1[0]){
            if(SB1[0]==1){
                Paso1++;//Sumamos un paso ya que ha habido un cambio de tendencia
            }
            SB1[1]=SB1[0];
        }

  if(abs(Sensor2[0]-Sensor2[1])<20){
            if(Sensor2[0]-Sensor2[1]<0)
                SB2[0]=1;
            else 
                SB2[0]=0;
        }
        if(SB2[1]!=SB2[0]){
            if(SB2[0]==1){
                Paso2++;//Sumamos un paso ya que ha habido un cambio de tendencia
            }
            SB2[1]=SB2[0];
        }
...
void ControlMotor::Motor(int velocidad, int giro){
  ...
  if( giro == 0){ // Mantenemos a 0 el angulo y contamos los pasos de cada motor
      if(Paso1-Paso2<0){
        
        analogWrite(pwmI,vel_izq +(Paso1-Paso2)*Kp);
        analogWrite(pwmD,vel_der +(Paso2-Paso1)*Kp); 
      }
