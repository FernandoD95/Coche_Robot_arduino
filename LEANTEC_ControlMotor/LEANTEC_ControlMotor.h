...
{
  public:
    ControlMotor(int MD1,int MD2,int MI1,int MI2,int PWMD,int PWMI,  int F1, int F2, float KpI);  // Asignamos los pines
    void Motor(int velocidad, int giro);  // Carga los valores en los motores
    void Actualizar_Paso(); //Ponemos el valor de los sensores fotoelectricos
  private:
    ...
    int F1;         // Esta variable la usa internamente la librería para el pin del fotodiodo 1
    int F2;         // Esta variable la usa internamente la librería para el pin del fotodiodo 2
    int Sensor1[1]; // Esta variable la usa internamente la librería para el fototransistor
    int Sensor2[1]; // Esta variable la usa internamente la librería para el fototransistor
    int Paso1;      // Esta variable la usa internamente la librería para contar pasos
    int Paso2;      // Esta variable la usa internamente la librería para contar pasos
    int SB1[1];     // Esta variable la usa internamente la librería para saber si estamos aumentando o disminuyendo el valor del fototransistor
    int SB2[1];     // Esta variable la usa internamente la librería para saber si estamos aumentando o disminuyendo el valor del fototransistor
    int Kp;         // Esta variable la usa internamente la librería para ajustar el avance de las 2 ruedas
};
